...

{
    ...
    citySearchWidget = nullptr;
    citiesCSVPath = "/home/pteck/Qgis_demo_withDB/Qgis_demo/worldcities/worldcities.csv";
...
void MainWindow::setupMenuBar()
{
    ...
    QAction *searchCityAction = toolsMenu->addAction(
                QIcon(":/icons/search.png"),
                "Search City...",
                this,
                &MainWindow::onSearchCity,
...
void MainWindow::setupToolBars()
{
    ...
    if (mapNavToolBar) {
        ...
        QToolButton *searchCityBtn = new QToolButton();
        searchCityBtn->setIcon(QIcon(":/icons/search.png"));
        searchCityBtn->setText("Search City");
        searchCityBtn->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
...
void MainWindow::onSearchCity()
{
    setupCitySearch();

    if (citySearchWidget) {
        citySearchWidget->setFocus();
        citySearchWidget->findChild<QLineEdit*>()->setFocus();
    }
}


void MainWindow::setupCitySearch()
{
    if (!citySearchWidget) {
        citySearchWidget = new CitySearchWidget(this);

        // Load cities
        QStringList possiblePaths = {
            "/home/pteck/Qgis_demo_withDB/Qgis_demo/worldcities/worldcities.csv",
            QDir::currentPath() + "/worldcities/worldcities.csv",
            QDir::currentPath() + "/../Qgis_demo/worldcities/worldcities.csv"
        };

        bool loaded = false;
        for (const QString &path : possiblePaths) {
            if (QFile::exists(path)) {
                if (citySearchWidget->loadCitiesFromCSV(path)) {
                    citiesCSVPath = path;
                    loaded = true;
                    qDebug() << "Loaded cities from:" << path;
                    break;
                }
            }
        }
        ...
        if (!loaded) {
            QString path = QFileDialog::getOpenFileName(
                this, "Select Cities CSV",
                "/home/pteck/Qgis_demo_withDB/Qgis_demo/worldcities/",
                "CSV Files (*.csv)"
            );

            if (!path.isEmpty()) {
                citySearchWidget->loadCitiesFromCSV(path);
            }
        }

        // Connect the signal
        connect(citySearchWidget, &CitySearchWidget::citySelected,
                this, [this](const CityData &city) {
            qDebug() << "Jumping to:" << city.name << city.country;
            jumpToLocation(city.longitude, city.latitude);

            if (messageLabel) {
                messageLabel->setText(QString("📍 %1, %2 (Lat: %3°, Lon: %4°)")
                    .arg(city.name).arg(city.country));
            }
        });

        // ADD TO TOOLBAR instead of status bar
        if (mapNavToolBar) {
            ...
            searchLabel->setStyleSheet("padding: 0 5px; font-weight: bold;");
            mapNavToolBar->addWidget(searchLabel);

            // Add the search widget
            mapNavToolBar->addWidget(citySearchWidget);
        }
    }
}
...
void MainWindow::setupConnections()
{
    ...
    QShortcut *searchShortcut = new QShortcut(QKeySequence("Ctrl+l"), this);
    connect(searchShortcut, &QShortcut::activated, this, [this]() {
        if (citySearchWidget) {
            QLineEdit *searchEdit = citySearchWidget->findChild<QLineEdit*>();
            if (searchEdit) {
                searchEdit->setFocus();
